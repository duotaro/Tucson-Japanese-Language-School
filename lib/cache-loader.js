...
import cacheConfig, { isCacheEnabledForType } from '../config/cache-config.js';
    ...
    if (processedItem.properties[key] && 
      ...
      if (fileUrl && fileUrl.startsWith('/image/download/')) {
        ...
        if (match) {
          ...
          // 同じファイル名が複数ある場合は識別子を追加
          baseName = originalName === 'profile' ? `${originalName}-${shortPageId}` : originalName;
        } else {
          baseName = file.name ? path.parse(file.name).name : fullBaseName;
        }
        ...
        const actualPagePath = dirMapping[dataType] || dataType;
        
        processedItem.properties[key] = {
          ...processedItem.properties[key],
          optimizedImage: {
            baseName: baseName,
            pagePath: actualPagePath,
 ...
 * @param {boolean} options.ignoreCache - キャッシュを無視して直接APIから取得するか
 * @returns {Promise<Array>} データ配列
 */
export async function loadCachedData(dataType, options = {}) {
  const {
    fallbackToAPI = cacheConfig.fallbackToAPI !== undefined ? cacheConfig.fallbackToAPI : true,
    maxAge = cacheConfig.maxAge || 24 * 60 * 60 * 1000, // デフォルト24時間
    ignoreCache = false // 個別にキャッシュを無視するオプション
  } = options;

  // キャッシュが無効化されているかチェック
  const useCache = !ignoreCache && isCacheEnabledForType(dataType);

  if (!useCache) {
    if (cacheConfig.debug) {
      console.log(`🚫 キャッシュ無効 (${dataType}): 直接APIから取得します`);
    }
