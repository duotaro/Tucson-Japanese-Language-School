  ...
  return text.map((value) => {
    ...
    return (
      <span
        ...
        key={text.content}
      >
        {text.link ? <a target="_blank" className="text-black hover:text-blue-500 underline hover:no-underline transition duration-300" href={text.link.url}>{text.content}</a> : text.content}
      </span>
    );
  });
};
